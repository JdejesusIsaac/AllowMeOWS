...
**Decision:** When a session completes, `complete-learning-session` triggers a separate LLM call (using the same Claude/ChatGPT client) to generate the parent receipt. The tutor's session transcript is the input; the receipt is the output.

**Why:** the tutor was advocating for the kid throughout the session. The receipt needs editorial distance — what the parent sees should be honest about both successes and struggles. Separating the calls is the simplest way to keep that distance.
...
### Workstream W6: Payout formula + settlement (~1 hour)

| Step | Task | Complexity | Est. |
|------|------|-----------|------|
| W6.1 | Extend `distribute-allowance` to accept engagement-weighted session-shaped payouts (vs the existing weekly-budget-distributed-by-category) | High | 45m |
| W6.2 | Audit entry `learning-session-completed` includes payout breakdown for transparency | Low | 15m |

### Workstream W7: Bootstrap form extension (~1 hour)
...
| W6 — Payout settlement | 1.0 | 13.0 |
| W7 — Bootstrap form | 1.0 | 14.0 |
| W8 — Tests | 3.0 | 17.0 |
| W9 — Docs + smoke | 1.0 | 18.0 |

**Total: ~18 hours.** Three focused days, or four 4-5 hour sessions. The math curriculum fragments (W2) and the engagement-scoring infrastructure (W4) are the time bombs. If pedagogy or prompt engineering surface unexpected complexity, those workstreams can grow. Receipt generation (W5) and payout (W6) are relatively bounded.
...
12. **Math-only enforcement.** A configure-policy call setting studyPlan on a non-math goal returns clear error: "Study plans are math-only in this version. Other subjects can be added as tracker-only goals." (Or: silently accepts and runs as tracker-only with a note.)
...
|----------|--------|---------------|
| Pedagogical quality (math fragments + LLM behavior) | 30% | The load-bearing IP of the sprint. Poor pedagogy = poor product regardless of engineering correctness. |
| Cheating defense robustness | 25% | Layers 1-5 + 7 must hold against typical-motivation gaming. Tested with Aiden as adversary-simulator during smoke. |
...
| Backward compat | 10% | Pre-4.0 families' goals must continue to work. |
| Mobile usability | 5% | Real iOS session completion is the smoke test. |

### Grading thresholds

- **Pass:** All success criteria 1–15 verified. No category below 70%. Aiden dogfooding produces at least one successful end-to-end session (baseline → tutoring → assessment → receipt → settlement) without manual intervention.
- **Fail:** Any of (1)–(15) fails. OR existing test suite regresses. OR Aiden's first session is unable to complete without manual intervention.
...
3. ✅ 30-min prompt-fragment experiment validates Learning Mode behavior
...
- [ ] Decision on receipt LLM provider (probably same as tutor LLM; verify no architectural assumption that they're different)